...
export const register = async (req, res) => {
    ...
    try {
        ...
        // EMAIL LOGIC (Non-blocking: Agar email na bhi jaye to registration na ruke)
        const mailOptions = {
            from: process.env.SENDER_EMAIL,
            to: email,
            subject: 'Welcome by Zain Ur Rehman',
            html: WELCOME_TEMPLATE.replace("{{name}}", name).replace("{{email}}", email)
        };

        // Hum yahan 'await' nahi lagayenge
        transporter.sendMail(mailOptions).catch(err => console.log("SMTP Error ignored:", err.message));

        return res.json({ success: true, message: "Registration Successful" });

    } catch (error) {
        // Yeh line aapko terminal mein batayegi ke ASAL masla kya hai
...
export const sendResetOtp = async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: 'Verification Send to your Email' })
